...
| SQS script/runtime mismatch | Provisioning script output did not match current runtime queue settings | Repository scripts default to `internship-outbox-events` and visibility timeout `60`; runtime context says `internship-prod-outbox` and visibility timeout `120` | Inspect `scripts/aws/provision-outbox-sqs.sh` and AWS queue attributes | Use runtime evidence for the report; update scripts only in a future application change task | Keep infrastructure scripts synchronized with production names |
